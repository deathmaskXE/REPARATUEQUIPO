...
$("crearCita").onclick=async()=>{try{
 const now=Date.now(),folio=`RTE-CITA-${String(now).slice(-6)}`;
...
async function generarPDF(x){
 ...
 p.setFillColor(...azul);p.roundedRect(13,13,184,39,3,3,"F");p.setFillColor(232,237,242);p.roundedRect(19,19,27,27,4,4,"F");p.setTextColor(...azul);p.setFont("helvetica","bold");p.setFontSize(19);p.text("RTE",32.5,37,{align:"center"});
 p.setTextColor(255,255,255);p.setFontSize(17);p.text("CONFIRMACIÓN DE CITA",55,30);p.setFont("helvetica","normal");p.setFontSize(8);p.text("REPARA TU EQUIPO · ATENCIÓN PROGRAMADA",55,40);
