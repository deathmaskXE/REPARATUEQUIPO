...
$("crear").onclick=async()=>{
  try{
    const now=Date.now();
    const folio=`RTE-${new Date().getFullYear()}-${String(now).slice(-6)}`;
    const d={
    ...
    if(citaOrigen?.id){
      ...
      localStorage.removeItem("rte_cita_recepcion");
      citaOrigen=null;
...
function equipoCompleto(d){
  ...
  if(!["consola","control"].includes(equipoNormal)&&(equipoNormal.includes(modeloNormal)||modeloNormal.includes(equipoNormal)))return equipo;
  return `${equipo} ${modelo}`;
}

function linkRecepcionPublica(folio){
  const pagina=new URL("index.html",location.href);
  pagina.searchParams.set("folio",folio);
  return pagina.href;
}
...
function enviarFolioWhatsApp(d){
  ...
  const mensaje=`🎮 *REPARA TU EQUIPO*
...
$("downloadBackup").onclick=async()=>{
  ...
  descargarArchivo(JSON.stringify(respaldo,null,2),selloArchivo("RTE-Respaldo","json"),"application/json;charset=utf-8");
  const avisos=[];
...
function abrirAvisoWhatsApp(d,ventana){
  ...
  const mensaje=`🎮 *ACTUALIZACIÓN DE REPARACIÓN RTE*
...
function pdfLogo(p,x,y,primario,oscuro=false){
  ...
  p.setTextColor(oscuro?20:255,oscuro?20:255,oscuro?20:255);p.setFont("helvetica","bold");p.setFontSize(19);p.text("RTE",x+13.5,y+18,{align:"center"});
}
function pdfPie(p,primario){
  p.setDrawColor(...primario);p.setLineWidth(.45);p.line(18,270,192,270);
  p.setTextColor(38,38,38);p.setFont("helvetica","bold");p.setFontSize(9);p.text("REPARA TU EQUIPO",105,278,{align:"center"});
...
const NOTA_CYAN=[220,24,35],NOTA_DARK=[3,8,10],NOTA_LINE=[135,145,150];
...
function pdfLogoFallback(p,x,y){
  p.setTextColor(...NOTA_CYAN);p.setFont("helvetica","bold");p.setFontSize(24);p.text("RTE",x,y+15);
...
function pdfNotaFooter(p){
  ...
  p.text("UBICACIÓN POR CONFIGURAR",84,289,{align:"center"});
  p.text("Repara Tu Equipo",170,289,{align:"center"});
}
...
const DATOS_TALLER={
  nombre:"Repara Tu Equipo",
  slogan:"Expertos en Tecnología",
  responsable:"POR CONFIGURAR",
  direccion:"POR CONFIGURAR",
  telefono:"POR CONFIGURAR",
  facebook:"POR CONFIGURAR",
  tiktok:"POR CONFIGURAR",
  youtube:"POR CONFIGURAR",
  maps:"POR CONFIGURAR"
};
...
function cargarLogoXE(){
  ...
  return new Promise((resolve,reject)=>{
    const img=new Image();
    img.onload=()=>{logoRteCache=img;resolve(img)};
    img.onerror=()=>reject(new Error("No se pudo cargar el logotipo RTE"));
    img.src="logo-rte.jpeg";
  });
}
...
async function generarPDFEntregaAnterior(x){
  ...
  try{
    ...
    p.addImage(logo,"JPEG",16,15,43,43,undefined,"FAST");
